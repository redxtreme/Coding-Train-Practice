...
function Spot(i, j) {
  ...
  this.addNeighbors = function(spotGrid) {
    var i = this.i;
    var j = this.j;
...
function solveSetup() {
  ...
  for (var i = 0; i < cols; i++) {
    for (var j = 0; j < rows; j++) {
      spotGrid[i][j].checkNeighbors(spotGrid);
    }
  }

  // Pick the start and end positions
  start = spotGrid[0][0];
  end = spotGrid[cols - 1][rows - 1];;
  start.wall = false;
  end.wall = false;
